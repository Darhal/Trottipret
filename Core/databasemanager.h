...
/*
* Une classe de gestionnaire de base de données (Design Pattern: Singleton) qui gérera les schémas
...
public:
    ...
    // Fonction qui initialise notre base de donnees
    void InitilizeDatabase();

    // Fonction facilitant l'exécution des requêtes sqlite
    QSqlQuery Exec(const char* statment);

    // Fonction facilitant l'exécution des requêtes sqlite avec des arguments variables de type different
...
private:
    ...
    DatabaseManager(); // constructeur privée car c'est une classe singleton
};
